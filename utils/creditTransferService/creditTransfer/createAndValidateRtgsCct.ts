...
export async function createAndValidateRtgsCct(
  rtgsPayload: any,
  senderToken: string,
  ...
  console.log("RTGS Creation Response:", JSON.stringify(createBody, null, 2));
  ...
  expect(createBody.creditor_amount).toBe(rtgsPayload.creditor_amount);
  expect(createBody.creditor_currency).toBe(rtgsPayload.creditor_currency);
  expect(createBody.creditor_xmi).toBe(rtgsPayload.creditor_xmi);

  const validationId = createBody.validation_id;
  console.log(`RTGS created successfully with validation ID: ${validationId}`);
