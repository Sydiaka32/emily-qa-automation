...
export async function createAndVerifyBelowMin(
  cctPayload: any,
  senderToken: string,
  expectedMinAmount: number,
): Promise<{ status: number; body: any; errorMessage?: string }> {
  console.log("Attempting to create CCT with amount below minimum...");
  console.log(`Expected minimum amount: ${expectedMinAmount}`);
  ...
  // Validate that the amount in the payload is indeed below minimum
  expect(cctPayload.creditor_amount).toBeLessThan(expectedMinAmount);
  console.log(
    `Amount ${cctPayload.creditor_amount} is below minimum ${expectedMinAmount}`,
  );
