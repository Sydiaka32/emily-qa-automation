...
/**
 * Get list of regions with assigned tariffs
 */
export async function getAssignedTariffsForRegionsBo(
  operatorToken: string,
  page: number = 0,
  size: number = 25,
): Promise<{ response: any; body: any }> {
  console.log("Getting list of regions with assigned tariffs...");
  ...
  console.log(
    `Assigned tariffs for regions request completed with status: ${result.response.status()}`,
  );
