...
test.describe("Matching Market buy IOC with single order by full volume", () => {
  ...
  test("Market buy IOC order should be fully filled by limit sell order", async () => {
    // 1. Create Market Buy IOC order payload (but don't send it yet)
    const marketOrderPayload = createOrder("MARKET", "IOC", "BID");

    console.log("Market Buy IOC Order Payload (to be used for precondition):");
    ...
    // 3. Now create the Market Buy IOC order (taker)
    console.log("Creating Market Buy IOC Order...");
    ...
    console.log(`Market Buy IOC order created with OUID: ${marketOrderOuid}`);
    ...
    expect(filledMarketOrder.status).toBe(OrderStatuses.filled);
    ...
    expect(filledMarketOrder.constraint).toBe("IOC");
    ...
    console.log(`Taker (Market Buy IOC):`);
