...
test.describe("Dynamic Limit Orders - GFD BID with Service Management", () => {
  ...
  test.beforeAll(async () => {
    ...
    // Configure services for Dynamic Limit orders
  ...
  test("New Dynamic Limit buy GFD - Order is created with LP service", async () => {
    // MAIN TEST: Create Dynamic Limit order
    console.log("\n=== Creating Dynamic Limit Order ===");

    // 1. Create the Dynamic Limit GFD BID order payload
    const orderPayload = createOrder("DYNAMIC_LIMIT", "GFD", "BID");

    console.log("Creating Dynamic Limit Buy GFD Order:");
    ...
    console.log(
      `Dynamic Limit GFD BID order created with OUID: ${createdOrderOuid}`,
    );
    ...
    console.log(`   - Type: Dynamic Limit GFD Buy`);
    console.log(
