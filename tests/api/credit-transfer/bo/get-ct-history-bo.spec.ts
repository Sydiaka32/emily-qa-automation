...
import { getCreditTransfersBo } from "@utils/creditTransferService/creditTransfer/bo/getCreditTransfersBo";

test.describe("Back Office - Credit Transfer History", () => {
  ...
  test("BO: Get list of transactions with default pagination", async () => {
    console.log("Testing BO credit transfers list with default pagination...");

    // Get transactions list with default parameters (page=0, size=10)
    const { body } = await getCreditTransfersBo(operatorToken);
    ...
    console.log("BO credit transfers list test completed successfully");
  });

  test("BO: Get list of transactions with custom pagination", async () => {
    console.log("Testing BO credit transfers list with custom pagination...");

    const page = 1;
    const size = 5;

    // Get transactions list with custom pagination
    const { body } = await getCreditTransfersBo(
      operatorToken,
      undefined,
      page,
      size,
  ...
  test("BO: Get list of transactions with search filter", async () => {
    console.log("Testing BO credit transfers list with search filter...");
    ...
    const { body } = await getCreditTransfersBo(operatorToken, searchTerm);
    ...
    console.log("BO search filter test completed successfully");
  });
});
